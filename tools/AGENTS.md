...
- `tools/dev` provides `@jt-design/tools-dev` and the `tools-dev` bin. It is the only currently active local development lifecycle control plane.
...
- `tools/pack` provides `@jt-design/tools-pack` and the `tools-pack` bin. The active slice is packaged artifact build/install/start/stop/logs/uninstall/cleanup/list/reset plus beta release artifact preparation for mac and Windows lanes, plus a Linux AppImage lane with optional containerized builds.
...
- Orchestration layers must consume primitives from `@jt-design/sidecar-proto`, `@jt-design/sidecar`, and `@jt-design/platform`.
...
```bash
pnpm --filter @jt-design/tools-dev typecheck
pnpm --filter @jt-design/tools-dev build
pnpm --filter @jt-design/tools-pack typecheck
pnpm --filter @jt-design/tools-pack build

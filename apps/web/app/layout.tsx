...
export const metadata: Metadata = {
  title: 'JT Design',
  description: 'AI-powered design studio — create stunning prototypes, decks, and design systems. tuatahi.art',
  icons: {
    icon: '/app-icon.svg',
    ...
    other: [{ rel: 'mask-icon', url: '/app-icon.svg', color: '#ff2d00' }],
  },
};
 ...
 */
const themeInitScript = `(function(){try{var c=JSON.parse(localStorage.getItem('jt-design:config')||localStorage.getItem('jt-design:config')||'{}');var t=c.theme;if(t==='light'||t==='dark')document.documentElement.setAttribute('data-theme',t);var a=typeof c.accentColor==='string'&&/^#[0-9a-fA-F]{6}$/.test(c.accentColor.trim())?c.accentColor.trim().toLowerCase():'';if(a){var s=document.documentElement.style;s.setProperty('--accent',a);s.setProperty('--accent-strong','color-mix(in srgb, '+a+' 86%, var(--text-strong))');s.setProperty('--accent-soft','color-mix(in srgb, '+a+' 22%, var(--bg-panel))');s.setProperty('--accent-tint','color-mix(in srgb, '+a+' 12%, var(--bg-panel))');s.setProperty('--accent-hover','color-mix(in srgb, '+a+' 90%, var(--text-strong))');}}catch(e){}})();`;

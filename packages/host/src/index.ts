...
/**
 * Converts a privileged host adapter's raw "replace working dir" result
 * into the host-owned renderer contract. Same boundary discipline as
 * `normalizeOpenDesignHostProjectImportResult`: only the daemon's
 * project-identifying fields cross.
 */
...
/**
 * Drives the desktop "pick + replace working directory" atomic flow.
 * Returns `unavailable` when no host bridge is present so callers can
 * fall back to the daemon's web-only path (which works only when the
 * desktop auth gate is dormant).
 */
